    ...
    <div class="panel">
      <div class="panel-header">
        <div class="page-title">
          <span>杩斾剑璁板綍</span>
          <el-tag type="success" effect="plain">鐪熷疄鎺ュ彛锛?report/commission/list + /report/commission/stats</el-tag>
        </div>
        <div class="header-actions">
          <el-radio-group v-model="rangeType" @change="handlePresetChange">
            <el-radio-button label="today">褰撴棩</el-radio-button>
            <el-radio-button label="yesterday">鏄ㄦ棩</el-radio-button>
            <el-radio-button label="last7">7鏃?/el-radio-button>
            <el-radio-button label="week">鏈懆</el-radio-button>
            <el-radio-button label="month">鏈湀</el-radio-button>
          </el-radio-group>
          <el-input v-model="search.user_id" clearable placeholder="鐢ㄦ埛ID" style="width: 120px" @keyup.enter="handleSearch" />
          <el-select v-model="search.money_type" clearable placeholder="杩斾剑绫诲瀷" style="width: 130px">
            <el-option label="涓€绾ц繑浣? :value="1" />
            <el-option label="浜岀骇杩斾剑" :value="2" />
            <el-option label="涓夌骇杩斾剑" :value="3" />
          </el-select>
          <el-select v-model="search.is_add_to_user_account" clearable placeholder="鍏ヨ处鐘舵€? style="width: 130px">
            <el-option label="鏈叆璐? :value="0" />
            <el-option label="宸插叆璐? :value="1" />
          </el-select>
          <el-date-picker
            v-model="customRange"
            type="datetimerange"
      ...
      <div class="summary-grid">
        <div v-for="item in periodCards" :key="item.key" class="summary-card period-card">
          ...
          <small>杩斾剑 {{ item.data.commission_user_count || 0 }} 浜?路 璁板綍 {{ item.data.record_count || 0 }} 鏉?/small>
        </div>
      </div>

      <div class="summary-grid current-grid">
        <div class="summary-card">
          <span class="summary-label">杩斾剑浜烘暟</span>
          <strong>{{ stats.commission_user_count || 0 }}</strong>
        </div>
        <div class="summary-card success">
          <span class="summary-label">杩斾剑閲戦</span>
          <strong>{{ currencyPrefix }}{{ formatMoney(stats.gift_amount || 0) }}</strong>
        </div>
        <div class="summary-card primary">
          <span class="summary-label">杩斾剑璁板綍鏁?/span>
          <strong>{{ stats.record_count || 0 }}</strong>
        </div>
      </div>

      <div class="breakdown-card">
        <div class="breakdown-title">杩斾剑绫诲瀷缁熻</div>
        <div class="breakdown-list">
          <div v-for="item in stats.reward_breakdown || []" :key="item.money_type" class="breakdown-item">
            <span>{{ item.money_type_text || `绫诲瀷${item.money_type}` }}</span>
            <span>{{ item.total_count }} 鏉?/span>
            <strong>{{ currencyPrefix }}{{ formatMoney(item.total_amount) }}</strong>
          </div>
          <el-empty v-if="!(stats.reward_breakdown || []).length" description="鏆傛棤缁熻" />
        </div>
      </div>

      <el-table v-loading="loading" :data="list" border>
        <el-table-column prop="id" label="ID" width="80" />
        <el-table-column prop="user_id" label="鐢ㄦ埛ID" width="100" />
        <el-table-column label="杩斾剑鐢ㄦ埛" min-width="220">
          <template #default="{ row }">
            <div class="info-cell">
              <span>鍚嶇О锛歿{ row.user_name || "-" }}</span>
              <span>鏄电О锛歿{ row.nickname || "-" }}</span>
              <span>鐢佃瘽锛歿{ row.phone || "-" }}</span>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="涓嬬骇鐢ㄦ埛" min-width="220">
          <template #default="{ row }">
            <div class="info-cell">
              <span>ID锛歿{ row.sub_id || "-" }}</span>
              <span>璐﹀彿锛歿{ row.sub_user_name || "-" }}</span>
              <span>鐢佃瘽锛歿{ row.sub_phone || "-" }}</span>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="money_type_text" label="杩斾剑绫诲瀷" min-width="120" />
        <el-table-column prop="money_amount" label="杩斾剑閲戦" min-width="120">
          <template #default="{ row }">{{ currencyPrefix }}{{ formatMoney(row.money_amount) }}</template>
        </el-table-column>
        <el-table-column prop="is_add_to_user_account" label="鍏ヨ处鐘舵€? min-width="100">
          <template #default="{ row }">
            <el-tag :type="Number(row.is_add_to_user_account) === 1 ? 'success' : 'warning'" effect="light">
              {{ Number(row.is_add_to_user_account) === 1 ? "宸插叆璐? : "鏈叆璐? }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="product_id" label="浜у搧ID" min-width="100" />
        <el-table-column prop="product_lev" label="浜у搧绛夌骇" min-width="100" />
        <el-table-column prop="remark" label="澶囨敞" min-width="220" show-overflow-tooltip />
        <el-table-column prop="create_time" label="鍒涘缓鏃堕棿" min-width="180" />
      </el-table>
...
const periodCards = computed(() => [
  { key: "today", title: "浠婂ぉ", data: periodStats.today },
  { key: "yesterday", title: "鏄ㄥぉ", data: periodStats.yesterday },
  { key: "week", title: "鏈懆", data: periodStats.week },
  { key: "month", title: "鏈湀", data: periodStats.month }
]);
